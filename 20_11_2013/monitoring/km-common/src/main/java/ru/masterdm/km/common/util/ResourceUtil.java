...
public class ResourceUtil {
	...
	static public Properties getAsProperties(String resourceName, ClassLoader classLoader) throws UtilRuntimeException {
		...
		try {
			...
			if (resourceStream == null) {
				resourceStream = ResourceUtil.class.getResourceAsStream(resourceName);
			}
			...
			if (resourceStream == null) {
				...
				System.err.println("Could not load properties from resource \"" + resourceName + "\".  Check the classpath.");
				throw new UtilRuntimeException("Could not find resource \"" + resourceName + "\".  Check the classpath.");
			}
			...
			throw new UtilRuntimeException("Could not load properties from resource \"" + resourceName + "\".  Check the classpath.", e);
		} finally {
			if (resourceStream != null) {
				try {
