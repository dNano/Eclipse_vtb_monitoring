...
public class EditPlanForDeal extends SimpleBasePage {
	...
	void onActivate(long eventPlanID, String returnPageName, long id) {
		// System.out.println("onActivate =" + eventPlanID + "; ID=" + id + "; returnPage=" + returnPageName);
		event = dealDao.getPlanEventForDeal(eventPlanID);
		if ("eventInstances/dealClassifier".equalsIgnoreCase(returnPageName)) {
			returnPage = pageRenderLinkSource.createPageRenderLinkWithContext(returnPageName, id, ClassifierBasePage.PLAN_TAB);
	...
	public java.util.List<ConditionTypeInstance> getEventConditions() {
		if (conditionList == null && event != null) {
			Long idEventType = event.getClassifier().getEventType().getId();
			conditionList = eventDao.getEventConditions(event.getDeal().getId(), idEventType);
		}
