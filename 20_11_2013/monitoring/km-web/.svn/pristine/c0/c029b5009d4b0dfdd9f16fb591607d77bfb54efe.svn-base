...
public abstract class ClassifierBasePage extends ClassifierTreeBasePage {
	...
	public JSONObject getOptions() {
		...
		oLanguage.put("sInfoEmpty", "Отображены записи от 0 до 0 из 0");
		oLanguage.put("sLengthMenu", "Отобразить _MENU_ записей");
		oLanguage.put("sInfoPostFix", "");
		oLanguage.put("sLoadingRecords", "Загрузка...");
		oLanguage.put("sInfoFiltered", "(отфильтровано из _MAX_ записей)");
		oLanguage.put("sSearch", "Поиск");
		oLanguage.put("sEmptyTable", "Таблица не содержит данных");
		oLanguage.put("sProcessing", "Обработка...");
		oLanguage.put("sInfo", "Отображены записи от _START_ до _END_ из _TOTAL_");
		oLanguage.put("sZeroRecords", "Соответствующие записи не найдены");
		JSONObject oPaginate = new JSONObject();
		oPaginate.put("sNext", "Следующая");
		oPaginate.put("sLast", "Последняя");
		oPaginate.put("sFirst", "Первая");
		oPaginate.put("sPrevious", "Предыдущая");
