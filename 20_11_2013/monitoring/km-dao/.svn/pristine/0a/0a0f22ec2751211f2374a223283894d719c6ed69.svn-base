...
public class DictionaryDaoImpl extends BaseJdbcDao<DictionaryDaoSqlKey> implements DictionaryDao {
	...
	@Override
	public InputStream getDocAsStream(String docID) {
		String query = sql.getValue(DictionaryDaoSqlKey.DOC_AS_STREAM);
		// return queryForInputStream(query, "Error get Document as Stream", docID);
	...
	@Override
	public InputStream getDocData(String docID) {
		String query = sql.getValue(DictionaryDaoSqlKey.DOC_AS_STREAM);
		// return queryForInputStream(query, "Error get Document as Stream", docID);
		InputStream stream = null;
		try {
