...
import ru.masterdm.km.dao.mapper.DealConditionRm;
...
public class EventDaoImpl extends BaseJdbcDao<EventDaoSqlKey> implements EventDao {
	...
	public List<KmEventInstance> getInstances(int startIndex, int amount, EventsByDateFilter filter, List<SortCriterion> sortCriteria) {
		...
		query += ", row_number() over (order by " + orderBy + ") rn from km_event ei left join users u on u.id_user = ei.id_executor "
				...
				+ "left join v_cps_credit_deal_number i on i.id_mdtask = e.id_mdtask "
	...
	private String filterQuery(EventsByDateFilter filter, String query, ArrayList<Object> params) {
		...
		if (filter.getExecutor() != null) {
			query += " AND ei.id_executor = ? ";
			params.add(filter.getExecutor().getId());
		}
		if (filter.getContractNumber() != null) {
			query += " AND lower(i.official_number ) like lower(?) ";
			params.add("%" + filter.getContractNumber().trim() + "%");
		}
	...
	@Override
	public List<ConditionTypeInstance> getEventConditions(Long dealID, Long idEventType) {
		String query = sql.getValue(EventDaoSqlKey.EVENT_CONDITIONS);
		List<DealCondition> listCondition = queryForList(query, new DealConditionRm(), "Error getEventConditions.", dealID, idEventType);
		List<ConditionTypeInstance> list = new ArrayList<ConditionTypeInstance>();
		ConditionTypeInstance curTypeIns = null;
		for (DealCondition dealCondition : listCondition) {
			if (curTypeIns == null || !curTypeIns.getType().getId().equals(dealCondition.getType().getId())) {
				curTypeIns = new ConditionTypeInstance();
				curTypeIns.setType(dealCondition.getType());
				curTypeIns.setConditions(new ArrayList<DealCondition>());
				list.add(curTypeIns);
			}
			curTypeIns.getConditions().add(dealCondition);
		}
		return list;
	}
}
