...
public class DealDaoImpl extends BaseJdbcDao<DealDaoSqlKey> implements DealDao {
	...
	private String filterQuery(EventsByDealFilter filter, String query, List<Object> params) {
		...
		if (filter.getSpkpNumber() != null) {
			...
			params.add("%" + filter.getSpkpNumber().trim() + "%");
		}
	...
	private String getOrderBy(List<SortCriterion> sortCriteria) {
		...
		if (!sortCriteria.isEmpty()) {
			...
			if ("spkpNumber".equalsIgnoreCase(propertyName)) {
				...
				// orderBy = "m.borrower_name"; здесь теперь список клиентов сделки.
	...
	@Override
	public List<ConditionTypeInstance> getDealConditions(long dealID) {
		String query = sql.getValue(DealDaoSqlKey.DEAL_CONDITIONS);
		List<DealCondition> listCondition = queryForList(query, new DealConditionRm(), "Error getDealConditions.", dealID);
